...
AirTranslate **1.14.0/build1140**는 메인 화면 우측 하단 컨트롤에서 제공자별 모델을 직접 선택할 수 있게 합니다. 콘솔 배지는 선택된 모델 이름을 보여 주며, 같은 팝오버에서 제공자 전환, 모델 선택, 설정 바로가기를 함께 다룹니다. 현재 워크플로에서 지원되면 같은 팝오버에서 번역 음성 모델을 선택할 수 있습니다.
