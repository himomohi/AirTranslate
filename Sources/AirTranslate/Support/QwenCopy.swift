...
enum QwenCopy {
    ...
    static let fileTranscriptionUnavailable = copy("QwenCloud could not complete the file transcription. Check the API key, workspace access, quota, and service status.", "QwenCloud 파일 전사를 완료하지 못했습니다. API 키·워크스페이스 권한·할당량·서비스 상태를 확인하세요.", "QwenCloudのファイル文字起こしを完了できませんでした。APIキー・ワークスペース権限・利用枠・サービス状態を確認してください。", "QwenCloud 无法完成文件转写。请检查 API 密钥、工作空间权限、配额和服务状态。")
